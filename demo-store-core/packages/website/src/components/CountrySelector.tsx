...
import type {
  RawDataCountry,
  RawDataLanguage,
} from "@commercelayer/demo-store-types";
...
export const CountrySelector: React.FC<Props> = ({ languages, countries }) => {
  ...
  const internationalLanguage = languages.find(
    (language) => language.code === NEXT_PUBLIC_DEFAULT_LANGUAGE
  );
    ...
    <div className="flex flex-col h-screen">
      <div className={styles.countriesContainer}>
        ...
        <div
          ...
          style={{
            gridTemplateColumns: groupedCountry.map(() => "1fr").join(" "),
          }}
        >
          {groupedCountry.map(([regionName, countries]) => (
            <Accordion
              key={regionName}
              title={
                <div className="font-extrabold">{regionName.toUpperCase()}</div>
              }
            >
              <div>
                {countries.map((country) => {
                  const locale = makeLocaleCode(
                    country.languages[0],
                    country.code
                  );
                  return (
      ...
      {internationalLanguage && (
        <div className="bg-gray-50 border-t border-gray-200 mt-12 pt-6 pb-6 flex">
          <div className="container mx-auto px-6">
            ...
            <Link
              locale={internationalLanguage.code}
