...
export const Page: React.FC<Props> = ({
  ...
  return (
    <>
      <Head>
        <title>{`${title ? `${title} - ` : ""}${
          settings.organization?.name || i18n.t("seo.title")
        }`}</title>
        <meta
          name="description"
          content={description || i18n.t("seo.description")}
        />
        <link
          rel="icon"
          href={
            settings.organization?.favicon_url ||
            NEXT_PUBLIC_BASE_PATH + "/favicon.ico"
          }
        />
        <link
          rel="manifest"
          href={
            settings.organization?.manifest ||
            NEXT_PUBLIC_BASE_PATH + "/manifest.json"
          }
        />
        {canonical && (
          <link
            rel="canonical"
            href={`${SITE_URL}/${router.query.locale}${canonical}`}
          />
        )}
        <style>{`
          :root {
...

        {
          ...
          localeCodes.map((code) => (
            <link
              ...
              rel="alternate"
              hrefLang={code}
              href={`${SITE_URL}${switchLocale(router.asPath, code)}`}
            />
          ))
        }
      </Head>
      ...
      <main className="flex flex-col h-screen">
        <PageTemplate navigation={navigation}>{children}</PageTemplate>
      </main>
    </>
  );
};
